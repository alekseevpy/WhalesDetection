        ...
        for file in tqdm(files, desc="Оценка модели"):
            if file.lower().endswith((".jpg", ".jpeg", ".png")):
                ...
                results = search_similar(
                    query_path, index, paths, model, shape, k=max(k_values)
                )
                if not results:
                    continue

                retrieved_labels = [
                    os.path.basename(os.path.dirname(p[0])) for p in results
                ]
                for k in k_values:
                    all_scores[k].append(
                        1 if true_label in retrieved_labels[:k] else 0
                    )
...
def evaluate_model(model, shape):
    train_index, train_paths = build_faiss_index(
        os.path.join("../whales_processed", "train"), model, shape
    )
    metrics = evaluation(
